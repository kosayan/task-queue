...
const SK=“task-queue-v1”,SOK=“task-queue-sort”,DK=“task-queue-defaults”,THK=“task-queue-theme”,TRK=“task-queue-trash”,HRK=“task-queue-habits”,DRK=“task-queue-dayreset”,LEK=“task-queue-locemojis”,LXK=“task-queue-lastexport”,TPK=“task-queue-todaypicks”,TDK=“task-queue-todaypickday”,BNK=“task-queue-bannercount”,ABK=“task-queue-autobackup”,CBK=“task-queue-colorblind”;
...
useEffect(()=>{
// Daily auto-backup: download json with fixed filename (overwrites previous)
const last=ld(ABK,0);
...
// Skip on very first install (let user try the app before auto-download)
if(last===0){sv(ABK,Date.now());sv(ABK+”-day”,todayKey);return}
// Download if different day and user has >= 3 tasks
...
const cutoff=30*24*36e5;const now=Date.now();
const stalled=tasks.filter(t=>!t.done&&t.type!==“wish”&&(now-t.createdAt)>cutoff&&!t.lastSeenStall||(now-(t.lastSeenStall||0))>(7*24*36e5)&&(now-t.createdAt)>cutoff&&!t.done);
const real=tasks.filter(t=>!t.done&&t.type!==“wish”&&(now-t.createdAt)>cutoff&&(!t.lastSeenStall||(now-t.lastSeenStall)>7*24*36e5));
return real[0]||null;
},[tasks,tick]);
...
{/* Stalled task nudge (30+ days untouched) */}
...
{showSettings&&<div style={{position:“fixed”,top:0,left:0,right:0,bottom:0,background:T.modal,zIndex:100,display:“flex”,alignItems:“flex-start”,justifyContent:“center”,padding:“60px 16px 20px”,overflowY:“auto”}} onClick={()=>setShowSettings(false)}><div style={{background:T.card,border:“1px solid “+T.brd,borderRadius:16,padding:20,width:“100%”,maxWidth:500}} onClick={e=>e.stopPropagation()}>
  ...
  <div style={{marginBottom:14}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0"}}>
      <span style={{fontSize:12,color:T.sub}}>色弱モード（青・黄・茶ベース）</span>
  ...
  <div style={{marginBottom:14}}><div style={{fontSize:10,fontWeight:700,color:T.mut,textTransform:"uppercase",letterSpacing:1,marginBottom:7,fontFamily:"'JetBrains Mono',monospace"}}>新規タスクのデフォルト</div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0"}}>
      <span style={{fontSize:12,color:T.sub}}>締切をデフォルトで有効化</span>
...
{focusTaskId&&(()=>{const ft=tasks.find(t=>t.id===focusTaskId);if(!ft){setFocusTaskId(null);return null}const fsubs=ft.subtasks||[];const fsd=fsubs.filter(s=>s.done).length;return(<div style={{position:“fixed”,top:0,left:0,right:0,bottom:0,background:T.bg,zIndex:280,display:“flex”,flexDirection:“column”,padding:“calc(24px + env(safe-area-inset-top)) 20px calc(24px + env(safe-area-inset-bottom))”,overflowY:“auto”}}>
  ...
  <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",gap:24}}>
    {ft.icon&&<div style={{fontSize:80,textAlign:"center"}}>{ft.icon}</div>}
    <div style={{fontSize:28,fontWeight:800,color:T.text,textAlign:"center",lineHeight:1.3,padding:"0 8px"}}>{ft.title}</div>
    ...
    {fsubs.length>0&&<div style={{background:T.inp,borderRadius:12,padding:16,border:"1px solid "+T.brd}}>
      ...
      {fsubs.map(sub=><div key={sub.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0"}}>
        ...
        <span style={{fontSize:15,color:sub.done?T.mut:T.text,opacity:sub.done?0.5:1,flex:1,lineHeight:1.4}}>{sub.title}</span>
      </div>)}
    </div>}
  </div>
...
function TaskCard({task,T,isDark,sortOrder,expanded,memoExp,dragging,draggable,isToday,isLatestDone,locEmojis={},onToggleExpand,onToggleMemo,onToggleDone,onEdit,onDelete,onUpdateSubtasks,onUpdateMemo,onQuickUpdate,onPostpone,onToggleToday,onDragStart,onDragMove,onDragEnd}){
...
{showQuickMenu&&<div style={{position:“fixed”,top:0,left:0,right:0,bottom:0,background:“rgba(0,0,0,0.6)”,zIndex:200,display:“flex”,alignItems:“center”,justifyContent:“center”,padding:16}} onClick={()=>setShowQuickMenu(false)}>
  ...
  <div style={{background:T.card,border:"1px solid "+T.brd,borderRadius:14,padding:14,minWidth:240,maxWidth:320}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      ...
      {!task.done&&<button style={{padding:"11px",borderRadius:8,border:"1px solid "+T.brd,background:"transparent",color:T.text,fontSize:13,fontWeight:600,cursor:"pointer"}} onClick={()=>{onPostpone();setShowQuickMenu(false)}}>⏰ 先送り ({task.deadline?"+1日":"3日後18時"})</button>}
